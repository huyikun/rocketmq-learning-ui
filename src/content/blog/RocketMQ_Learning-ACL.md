...
<a name="Lv69W"></a>
### ACL 1.0 痛点问题
![](https://img.alicdn.com/imgextra/i2/O1CN01Prbqfi1ZxaXrsgRgG_!!6000000003261-49-tps-1080-1196.webp)<br />RocketMQ ACL 1.0 的认证和授权流程如上图所示，在使用过程中，存在着以下痛点问题：
...
<a name="erftt"></a>
### ACL 2.0 新特性
![](https://img.alicdn.com/imgextra/i4/O1CN01SyBHxD1fMqSxqTJLb_!!6000000003993-0-tps-2084-1622.jpg)<br />RocketMQ ACL 2.0 针对 ACL 1.0 中的问题进行了解决，同时还带来了六个主要的新特性，具体如下：
...
<a name="n6sKt"></a>
#### 主体模型
![](https://img.alicdn.com/imgextra/i4/O1CN01DGC5Bl1MTjj5jYegR_!!6000000001436-0-tps-2086-974.jpg)<br />为了提升 RocketMQ 系统的访问控制和权限管理，ACL 2.0 针对主体模型做了以下改进和扩展：
...
<a name="bekmn"></a>
#### 认证流程
![](https://img.alicdn.com/imgextra/i4/O1CN01TNax3o1QKM4nbW4PX_!!6000000001957-49-tps-1080-653.webp)<br />客户端流程：
...
<a name="KHKIG"></a>
#### 核心概念
授权作为一种安全机制，旨在确定访问请求者是否拥有对特定资源进行操作的权限。简而言之，授权就是在资源被访问之前回答“谁在何种环境下对哪些资源执行何种操作”这个问题。<br />基于“属性的访问控制（ABAC）”模型，RocketMQ ACL 2.0 涵盖了以下一系列的核心概念。在系统实现中，都会以以下概念作为指导，完成整个权限管理和授权机制的设计和实现。<br />![](https://img.alicdn.com/imgextra/i2/O1CN01MJcd2C1lMk4SXQ85n_!!6000000004805-49-tps-1080-1078.webp)
<a name="iJWW7"></a>
#### 权限模型
![](https://img.alicdn.com/imgextra/i3/O1CN016jlvy81mB7dg7WbBF_!!6000000004915-0-tps-2080-1172.jpg)<br />基于属性的访问控制（ABAC）模型的核心概念，ACL 2.0 对权限模型做了精心的设计，要点如下：
...
<a name="NGG5m"></a>
#### 授权流程
![](https://img.alicdn.com/imgextra/i4/O1CN01AC2iKX1oat9SQS4Yg_!!6000000005242-49-tps-1080-698.webp)<br />客户端流程：
...
<a name="Z8mzA"></a>
#### 授权参数的解析
在 ACL 2.0 中，更具操作类型和请求频率，对授权相关参数（包括资源、操作等）的解析进行了优化。<br />1. 硬编码方式解析<br />![](https://img.alicdn.com/imgextra/i1/O1CN01GXJcHp1m7STDVCzPp_!!6000000004907-49-tps-1080-288.webp)<br />对于消息发送和消费这类接口，参数相对较为复杂，且请求频次也相对较高。考虑到解析的便捷性和性能上的要求，采用硬编码的方式进行解析。<br />2. 注解方式解析<br />![](https://img.alicdn.com/imgextra/i1/O1CN0181NniO1Hpeb8dEQAa_!!6000000000807-2-tps-1080-216.png)<br />对于大量的管控接口，采用硬编码的方式工作量巨大，且这些接口调用频次较低，对性能要求不高，所以采用注解的方式进行解析，提高编码效率。
<a name="FGOhP"></a>
#### 权限策略优先级
在权限策略匹配方面，由于支持了模糊的资源匹配模式，可能出现同一个资源对应多个权限策略。因此，需要一套优先级的机制来确定最终使用哪一套权限策略。<br />![](https://img.alicdn.com/imgextra/i1/O1CN01Qd8kIH1FSB4FgsyUp_!!6000000000485-49-tps-1080-238.webp)<br />假设配置了以下授权策略，按照以上优先级资源的匹配情况如下：<br />![](https://img.alicdn.com/imgextra/i2/O1CN01ZC0bKc1zwpuBFHDAb_!!6000000006779-49-tps-1080-347.webp)
<a name="GioGK"></a>
### 认证授权策略
出于安全和性能的权衡和考虑，RocketMQ ACL 2.0 为认证和授权提供了两种策略：无状态认证授权策略（Stateless）和有状态认证授权策略（Stateful）。<br />![](https://img.alicdn.com/imgextra/i2/O1CN01wB9JLp25NvPQP7h1j_!!6000000007515-49-tps-1080-382.webp)
...
<a name="F8faf"></a>
### 插件化机制
为了适应未来持续发展的认证鉴权方式，以及满足用户针对特定场景的定制需求，RocketMQ ACL 2.0 在多个环节上提供了灵活性和可扩展性。<br />![](https://img.alicdn.com/imgextra/i2/O1CN01OFlzM41r3rRCC8LRh_!!6000000005576-49-tps-1080-552.webp)
...
<a name="jxeQD"></a>
#### 存算一体架构
![](https://img.alicdn.com/imgextra/i2/O1CN01dh44IJ1mjvjtU8dQ8_!!6000000004991-49-tps-1080-481.webp)<br />在 RocketMQ 存算一体架构中，Broker 组件同时承担了计算和存储的职责，并对外提供服务，接收所有客户端的访问请求。因此，由 Broker 组件承担认证和授权的重要角色。此外，Broker 组件还负责认证和授权相关的元数据的维护和存储。
<a name="uVLwY"></a>
#### 存算分离架构
![](https://img.alicdn.com/imgextra/i4/O1CN01VZhqG21t29Bfn475M_!!6000000005843-49-tps-1080-476.webp)<br />在 RocketMQ 存算分离架构中，存储由 Broker 组件负责，计算由 Proxy 组件负责，所有的对外请求都是由 Proxy 对外进行服务。因此，请求的认证和授权都由 Proxy 组件承担。Broker 承担元数据存储，为 Proxy 组件提供所需的认证和授权元数据的查询和管理服务。
<a name="ju5GA"></a>
### 集群配置
<a name="ngpzz"></a>
#### 认证配置
...
想要在服务端开启认证功能，相关的参数和使用案例主要包含如下：<br />![](https://img.alicdn.com/imgextra/i2/O1CN01Ixmeu51Iz6O0k71eV_!!6000000000963-49-tps-1080-1648.webp)
...
想要在服务端开启授权功能，相关的参数和使用案例主要包含如下：<br />![](https://img.alicdn.com/imgextra/i4/O1CN01B4XUUs297DQFDK6aR_!!6000000008020-49-tps-1080-995.webp)
...
![](https://img.alicdn.com/imgextra/i3/O1CN01eDVN5r1cUgObLs5k5_!!6000000003604-49-tps-1080-561.webp)
...
![](https://img.alicdn.com/imgextra/i3/O1CN01AIMm5N1NRHDK2GNuJ_!!6000000001566-49-tps-1080-564.webp)
...
<a name="w3Y4q"></a>
#### 扩容
如果想要在运行过程中的集群扩容一台 Broker，就需要将所有的元数据都同步到这台新的 Broker 上，ACL 2.0 提供了相应的拷贝用户和拷贝授权的接口来支持这项操作。<br />![](https://img.alicdn.com/imgextra/i1/O1CN01BDiOMT1MUeWBJtUKN_!!6000000001438-49-tps-1080-448.webp)

- 接口定义

![](https://img.alicdn.com/imgextra/i2/O1CN01hfzU5p1ehAQKCMQDD_!!6000000003902-49-tps-1080-249.webp)
...
<a name="BAGvo"></a>
#### 迁移
如果已经使用上了 ACL 1.0，想要无缝地迁移至 ACL 2.0，也提供了相应的解决方案，只需要做以下配置即可。<br />![](https://img.alicdn.com/imgextra/i4/O1CN014oTe8U1YnDxtTYNDa_!!6000000003103-49-tps-1080-426.webp)
